...
class AMQPSubscriber {
     ...
     */
    async bindQueue(routingKey, options) {
        const channel = await this.getOrCreateChannel();
        async function setupQueue(ch) {
            const queue = await this.setupQueueAndBind(ch, routingKey, options);
            this.logger('Created and bound Queue "%s" to exchange "%s" with routing key "%s"', queue.queue, this.exchange.name, routingKey);
            return queue.queue;
        }
     ...
     */
    async setupQueueAndBind(ch, routingKey, options) {
